...
# The desktop file's basename must equal the app-id (APP_ID in tuple_panel.py)
# so GNOME/Wayland can match the running window to it and show our icon.
rm -f "$APP_DIR/tuple-panel.desktop"  # drop the old mismatched name from past installs
install -m 0644 "$SRC_DIR/data/app.tuple.Panel.desktop" "$APP_DIR/app.tuple.Panel.desktop"
...
sed -i "s|^Exec=.*|Exec=$BIN_DIR/tuple-panel|" "$APP_DIR/app.tuple.Panel.desktop"
...
echo "  $APP_DIR/app.tuple.Panel.desktop"
